...
The precipitation datasets contains information about precipation for the year 2016 and 2017. There are `r nrow(prec_2016)` rows (observations) x `r ncol(prec_2016)` columns in the 2016 precipation data. In the dataset for 2017, we have `r nrow(prec_2017)` rows (observations) x `r ncol(prec_2017)` columns. Both contain information on total precipation (total variable) of each month (month variable).

The total precipitation in 2017 is `r sum(prec_2017$total)` and the median number of sports balls is `r  median(balls_median_16$sports_balls)`
...
There are `r nrow(distinct(brfss_df, locationdesc))` distinct locations. There are `r nrow(distinct(brfss_df, locationabbr))` distinct states. Every state is represented and Washington DC is also included. 
